...
- If Companion says the host does not support exact updates, use SillyTavern's extension manager. Standard SillyTavern releases do not currently expose Companion's exact-update service; Companion will not substitute the host's newest-only update command.
