...
function sameManagedRecord(current, observed) {
  return Boolean(
    current && current.projectId === observed.projectId && current.internalName === observed.internalName && current.folderName === observed.folderName && current.installedAt === observed.installedAt && current.installedBy === observed.installedBy && sameProvenance(current.provenance, observed.provenance)
  );
}
function sameProvenance(current, observed) {
  if (!current || !observed) return current === observed;
  return current.targetKind === observed.targetKind && current.requestedSha === observed.requestedSha && current.installedSha === observed.installedSha && current.catalogGeneratedAt === observed.catalogGeneratedAt && current.tavernKeeperReportId === observed.tavernKeeperReportId;
}
...
function KitVersionChoices({
  ...
  const hasVersionChoice = steps.some((step2) => step2.targetChoice?.kind === "choose");
  return /* @__PURE__ */ u3("section", { class: "tavernary-companion-kit-version-choices", "aria-labelledby": "kit-versions-heading", children: [
    ...
    /* @__PURE__ */ u3("p", { children: hasVersionChoice ? "Choose a version for each project that has two options." : "Companion will install the versions shown below." }),
...
function targetDescription(target) {
  ...
  return target.requestedSha === null ? "SillyTavern will install the creator\u2019s current version." : LATEST_CREATOR_DESCRIPTION;
}
...
function dispatchPreparedUpdateChoice(choice, onUpdate, onChoose) {
